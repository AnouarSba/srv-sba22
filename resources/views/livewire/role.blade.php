   ...
   <hr>
    ...
    <table class="table table-striped table-sm" style="margin-top:10px;">
        ...
        @foreach($users as $row)
           @if ($row->id != 1 || auth()->user()->hasrole('super_admin'))
            <tr>
                <td>{{$row->id}}</td>
                <td>{{$row->name}}</td>
                ...
                <td>
                    ...
                    @can('give_permission')
                    <button data-toggle="modal" data-target="#permModal" wire:click="edit({{ $row->id }})" class="btn btn-primary btn-sm">Permissions</button>
                    @endcan
    ...
    <div wire:ignore.self class="modal fade" id="permModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Modal title</h5>
               ...
               <div class="modal-body">
                    <form>
                        ...
                        <table class="table table-striped table-sm" style="margin-top:10px;">
                            ...
                            @foreach($perm as $pe)
                                <tr>
                                    <td><input wire:model.lazy="per" type="checkbox"  value="{{$pe->name}}"></td>
                                    <td>{{$pe->name}}</td>
                                </tr>
                            @endforeach
                        </table>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" wire:click.prevent="perm()" class="btn btn-primary close-modal">Save changes</button>
                </div>
            </div>
        </div>
    </div>
