...
<body style="font-family:Arial" >
 ...
 <div class="row">
    ...
    <table class="table table-bordered table-striped table-sm">
                   ...
                   @foreach ($et as $lig)
                          {{
                            $bus=  App\Models\V_ticket::where('skey',$en)->where('ligne_id',$lig['ligne_id'])->whereBetween('created_at', [$from, $to])->groupBy('bus_id')->get('bus_id');
                          }}
                        @foreach ($bus as $bu)

                           @php
                          $forn =  App\Models\V_ticket::whereBetween('created_at', [$from, $to])->where('skey',$en)->where('ligne_id',$lig['ligne_id'])->where('bus_id',$bu['bus_id'])->groupBy('Forn_id')->get('Forn_id');
                           @endphp
                           @foreach ($forn as $fo)
                                @php
                                  $tikk =  App\Models\V_ticket::whereBetween('created_at', [$from, $to])->where('skey',$en)->where('ligne_id',$lig['ligne_id'])->where('bus_id',$bu['bus_id'])->where('Forn_id',$fo['Forn_id'])->get();
                    ...
                    <tfoot>
                 {{ $tikk1 =  App\Models\V_ticket::whereBetween('created_at', [$from, $to])->where('skey',$en)->get();}}
                        <th>Total</th>
...
<div class="row">
    ...
    <table class="table table-bordered table-striped table-sm">
        ...
        {{$bus=  App\Models\V_ticket::where('skey',$en)->whereBetween('created_at', [$from, $to])->groupBy('bus_id')->get()}}
        <tbody>
           @foreach ($bus as $buss)
           @php
           $buss1=  App\Models\V_ticket::where('skey',$en)->whereBetween('created_at', [$from, $to])->where('bus_id',$buss['bus_id'])->get();
           @endphp
             <tr>
...
<div class="row">
    ...
    <table class="table table-bordered table-striped table-sm">
        ...
        {{$bus=  App\Models\V_ticket::where('skey',$en)->whereBetween('created_at', [$from, $to])->groupBy('ligne_id')->get()}}
        <tbody>
           @foreach ($bus as $buss)
           @php
           $buss1=  App\Models\V_ticket::where('skey',$en)->whereBetween('created_at', [$from, $to])->where('ligne_id',$buss['ligne_id'])->get();
           @endphp
             <tr>
