...
class StatController extends Controller
{
    ...
    public function  stat1(Request $request)
    {
        ...
        if ($req['type'] == Control::class) {
            ...
            $solde = Sold_cart::whereBetween('sold_carts.updated_at', [$from, $to])
                ->where('valid', false)->join('ps_carts', 'sold_carts.ps_id', '=', 'ps_carts.id')
    ...
    public function  stat_site($start, $end)
    {
        $from = $start;
        $to = $end;
        $cartvend = Vent::whereBetween('created_at', [$from, $to])->count();
        $flixy = Flixy::whereBetween('created_at', [$from, $to])->get();
        $flixy_sum = $flixy->sum('amount');
        $et = Spcart::where('type', "ELEV")->whereBetween('created_at', [$from, $to])->count();
        return response()->json([$flixy_sum, $cartvend, $et]);
    }
    public function  stat_site2($start, $end)
    {
        // $from = $start;
        // $to = $end;
        $from = Carbon::parse($start)->startOfDay();
        // $to   = Carbon::parse($end)->addDay()->endOfDay();
        $to   = Carbon::parse($end)->endOfDay();
        $ht = Cont::where('spcart_id', '!=', null)->with(['spcart'])->whereRelation('spcart', 'type', '=', 'HAND')->whereBetween('updated_at', [$from, $to]);
        $en = config('etus.server.code');
        $en = config('etus.server.code');
        $etRaw = E_ticket::where('skey', $en)
            ->where('client_type', '=', 'App\Models\Spcart')
        ...
        $raw = E_ticket::where('skey', $en)->whereBetween('created_at', [$from, $to])->groupBy('ligne_id','day',  'amount')->select(
            DB::raw('DATE(created_at) as day'),
            'ligne_id',
            'amount',
            DB::raw('SUM(amount) as somme')
        )->orderBy('day')
            ->get();
        // $raw = Frod::whereBetween('created_at', [$from, $to])->groupBy('ligne_id','day',  'amount')->select(
        //     DB::raw('DATE(created_at) as day'),
        //     'ligne_id',
        //     'amount',
        //     DB::raw('SUM(amount) as somme')
        ...
        foreach ($period as $date) {
            ...
            foreach ($lineAmounts as $ligne => $amounts) {
                foreach ($amounts as $amount) {
                    $tmp_data = $data
                        ->where('day', $day)
                        ->where('ligne_id', $ligne)
                        ->where('amount', $amount)
                        ->first();

                    $row[] = $tmp_data ? (int) $tmp_data['somme'] : 0;
                }
            }
